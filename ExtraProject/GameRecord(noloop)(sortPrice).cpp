...
int main()
{
	...
	int select = 1, i = 0, found = 0, DisplayID, update = 0, comp,temp, count=0, limit;
	char cont,search[50];
	vector<int> vec;
	vector<int> PriceSort;
	...
	for (int i = 0; select != 0; i++)
	{
		...
		else if (select == 3)				//prob: if game same price, it wont show //display the same item
		{
			if (vec.size() > 0)
			{
			
				vector<int>::iterator Priceitr;
				vector<int>::iterator itr;
				for (itr = vec.begin(); itr != vec.end(); itr++)
				{
					i = *itr;
					PriceSort.push_back(obj[i].price);
				}

				sort(PriceSort.begin(), PriceSort.end());
				cout << " ========= RECORD =========" << endl;
				for (Priceitr = PriceSort.begin(); Priceitr != PriceSort.end(); Priceitr++)
				{
					for (itr = vec.begin(); itr != vec.end(); itr++)
					{
						i = *itr;
						if (*Priceitr == obj[i].price)
						{
				...
				for (Priceitr = vec.begin(); Priceitr != vec.end(); Priceitr++)
				{
					i = *Priceitr;
					PriceSort.pop_back();
				}
